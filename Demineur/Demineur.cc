...
/* nb_try, nb_bomb, taille de la grille */
 Demineur::Demineur(const std::size_t count, const std::size_t bomb, const std::size_t gridsize){ 
...
void Demineur::reveal(std::vector<std::vector<bool>>&tmp, const std::vector<std::vector<int>>tab, const std::size_t i, const std::size_t j){
    if(!tmp[i][j] && !tab[i][j])
    {       
        tmp[i][j]=true;
