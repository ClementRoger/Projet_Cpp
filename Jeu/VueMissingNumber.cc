...
void Missingnumber:: init_background(sf::RenderWindow &window){
    ...
    sf::Texture texture;
    sf::Sprite sprite;
    texture.loadFromFile(TEXTURE_BACKGROUND2);
    sprite.setTexture(texture);
    window.draw(sprite);

    texture.loadFromFile(TEXTURE_LETTERS2);
    texture.setSmooth(true);
    sprite.setTexture(texture);
    sprite.setPosition(sf::Vector2f((APP_SIZE_X2-580)/2,(APP_SIZE_Y2-307)/2-10));
    window.draw(sprite);

    
    texture.loadFromFile(TEXTURE_GRID2);
    sprite.setPosition(sf::Vector2f((APP_SIZE_X2-348)/2,185)); 
    sprite.setTexture(texture);
    window.draw(sprite);
    ...
    text.setPosition(sf::Vector2f(APP_SIZE_X2/3+30,450));
    window.draw(text);
    ...
    texture.loadFromFile(TEXTURE_BACKNAME2);
    sprite.setTexture(texture);
    ...
    texture.loadFromFile(TEXTURE_NAME2);
    sprite.setPosition(sf::Vector2f(APP_SIZE_X2/2-260,15)); 
    sprite.setTexture(texture);
    window.draw(sprite);
}
...
text.setFont(font);
  for (std::size_t i = 0; i < get_matrice().size(); i++){
    for (std::size_t j = 0; j < get_matrice().size(); j++){
        ...
        text.setFillColor(sf::Color::White);
        text.setStyle(sf::Text::Bold);
        text.setPosition(sf::Vector2f( (APP_SIZE_X2-348)/2-5 + (2*i+1)*58 , 170 + (2*j+1)*25 ));
        window.draw(text); 
...
void Missingnumber::print_end(bool winner,sf::RenderWindow &window){
    ...
    sf::Sprite sprite;
    if (winner){ 
        texture.loadFromFile(TEXTURE_WIN2);
        sprite.setPosition(sf::Vector2f(530/2,411/2)); }
    else{ 
        texture.loadFromFile(TEXTURE_LOSE2); 
